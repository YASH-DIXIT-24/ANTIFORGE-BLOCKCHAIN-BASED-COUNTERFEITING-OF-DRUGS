...
contract Medicine {
    ...
    function receivedMedicine(address _receiverAddr) public returns (uint256) {
        require(
        ...
        if (_receiverAddr == wholesaler && status == medicineStatus(1)) {
            ...
            _receiverAddr == distributor && status == medicineStatus(2)
        ) {
    ...
    function receivedWtoD(address receiver) public {
        require(distributor == receiver, "This Distributor is not Associated.");
        status = medicineStatus(4);
    }

    function sendDtoC(address receiver, address sender) public {
        require(distributor == sender, "This Distributor is not Associated.");
        customer = receiver;
